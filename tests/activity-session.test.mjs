...
test('reading-session records preserve wall-clock bounds and active seconds', () => {
  ...
  assert.equal(rows[0].kind, 'reading-session');
...
test('reading-session records split safely at local midnight', () => {
  const rows = sessionRecords({ id: 'session-2', itemId: 'item-2', title: 'Item', startedAt: new Date(2026, 7, 31, 23, 50).getTime(), endedAt: new Date(2026, 8, 1, 0, 10).getTime(), activeSeconds: 1200 }, 'reading-session');
...
test('idle boundary ends the session and caps activeSeconds at the idle limit', () => withTracker(() => {
  ...
  const tracker = createSessionTracker({
    kind: 'reading-session', itemType: 'document', storageKey: 'test.idle',
...
test('background then resume produces two sessions, not one merged range', () => withTracker(() => {
  ...
  const records = [];
  const tracker = createSessionTracker({
    kind: 'reading-session', itemType: 'document', storageKey: 'test.bg',
    onRecord: (r) => records.push(r), now: clock.now, isVisible: () => visible,
  });
  tracker.start({ id: 'doc-1', title: 'Doc' }, clock.now());
