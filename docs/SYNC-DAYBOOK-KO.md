...
1. 이미 갖고 계신 `webapp-data-rw` 토큰(Tide 등 다른 앱에서 쓰시던 것과 같은 값)을
   그대로 사용합니다. 새로 발급받지 않아도 됩니다.
...
3. **Token (GitHub Personal Access Token)** 칸에 0단계의 `webapp-data-rw` 토큰을
   붙여넣고 **Save token**을 누릅니다.
...
3. **같은 `webapp-data-rw` 토큰**을 다시 붙여넣고 **Save token**을 누릅니다. (Safari와
   저장공간이 분리되어 있어 토큰도 다시 넣어야 합니다.)
...
4. 이후로는 앱을 열거나 화면으로 돌아올 때마다(홈 화면 앱을 배경에서 다시 열 때 포함)
   자동으로 최신 기록을 받아오고(pull), 앱을 배경으로 보내거나 닫을 때 바뀐 내용을
   자동으로 올려보냅니다(push). 평소에는 **Sync now**를 따로 누르지 않아도 두 화면이
   맞춰집니다 — 지금 바로 확인하고 싶을 때만 누르면 됩니다.
...
- 이 토큰은 Tide 등 다른 앱과 같은 `webapp-data` 비공개 저장소를 공유합니다 — 저장소
  안에서 앱마다 폴더(`cove/`, `tide/` 등)가 나뉘어 있어 서로의 데이터를 섞지는 않습니다.
- **Journal**(Daybook 연동) 기능도 같은 `webapp-data` 저장소·같은 토큰을 씁니다. Sync를
  먼저 켜 두면 Journal을 켰을 때 바로 정상 동작합니다.
